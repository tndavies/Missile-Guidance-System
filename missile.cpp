...
void Missile::tick(const Target& target, float dt)
{
	...
	m_LOS = calcLOSnorm(tpos);
	float currLOSA = glm::angle(m_InitialLOS, m_LOS);

	// 2) Calc temporal ROC of LOS angle.
	float rocLOSA = (currLOSA - m_prevLOSAngle) / dt;
	...
	auto sign = std::sinf(glm::angle(m_Vel, m_LOS));
